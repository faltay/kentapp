...
@section('title', __('admin.contractors.edit') . ': ' . $contractor->name)
...
<div class="page-header mb-4">
    <div class="row align-items-center">
        <div class="col">
            ...
            <h2 class="page-title">{{ __('admin.contractors.edit') }}: {{ $contractor->name }}</h2>
        </div>
        <div class="col-auto ms-auto">
            <a href="{{ route('admin.contractors.show', $contractor) }}" class="btn btn-secondary">
...
<form id="edit-contractor-form" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ $contractor->name }}">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" name="phone" class="form-control" value="{{ $contractor->phone }}" placeholder="+90 5xx xxx xx xx">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">{{ __('admin.contractors.form.company_name') }}</label>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">{{ __('admin.contractors.form.authorized_name') }}</label>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">{{ __('admin.contractors.form.company_phone') }}</label>
                        ...
                        <div class="col-md-6">
                            <label class="form-label">{{ __('admin.contractors.form.company_email') }}</label>
                        ...
                        <div class="col-md-12">
                            <label class="form-label">{{ __('admin.contractors.form.working_neighborhoods') }}</label>
                            @php $areas = $profile?->working_neighborhoods ?? []; @endphp
                            <select id="working-neighborhoods-select" name="working_neighborhoods[]" multiple
                                    placeholder="{{ __('admin.contractors.form.area_placeholder') }}">
                                @foreach($areas as $area)
                                    @php
                                        $val   = is_array($area) ? json_encode($area, JSON_UNESCAPED_UNICODE) : $area;
                                        $label = is_array($area)
                            ...
                            <div class="form-hint">{{ __('admin.contractors.form.area_hint') }}</div>
                            <div class="invalid-feedback" data-field="working_neighborhoods"></div>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">{{ __('admin.contractors.form.company_address') }}</label>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h3 class="card-title">
                        ...
                        {{ __('admin.contractors.form.certificate_status') }}
                    </h3>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <select name="certificate_status" class="form-select">
                            <option value="none"     @selected(($profile?->certificate_status ?? 'none') === 'none')>{{ __('admin.contractors.certificate.none') }}</option>
                            <option value="pending"  @selected($profile?->certificate_status === 'pending')>{{ __('admin.contractors.certificate.pending') }}</option>
                            <option value="approved" @selected($profile?->certificate_status === 'approved')>{{ __('admin.contractors.certificate.approved') }}</option>
                            <option value="rejected" @selected($profile?->certificate_status === 'rejected')>{{ __('admin.contractors.certificate.rejected') }}</option>
                        </select>
                        <div class="invalid-feedback" data-field="certificate_status"></div>
                    </div>
                    <div>
                        <label class="form-label">{{ __('admin.contractors.form.certificate_number') }}</label>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h3 class="card-title">
                        ...
                        {{ __('admin.contractors.form.certificate_file') }}
                    </h3>
                </div>
                <div class="card-body">
                    @php
                    ...
                    @include('admin.contractors.partials.certificate-dropzone', [
                        'existingUrl'  => $certMedia?->getUrl(),
                        'existingName' => $certMedia?->file_name,
                    ])
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="mb-2">
                        <label class="form-check form-switch">
                            <input type="checkbox" name="is_active" value="1" class="form-check-input" @checked($contractor->is_active)>
                            <span class="form-check-label">{{ __('admin.users.form.is_active') }}</span>
                        </label>
                    </div>
                    <div>
                        <label class="form-check form-switch">
                            <input type="checkbox" name="is_suspended" value="1" class="form-check-input" @checked($contractor->is_suspended)>
...
new TomSelect('#working-neighborhoods-select', {
    ...
    render: {
        option_create: function(data) {
            return '<div class="create">{{ __('admin.contractors.form.area_add_custom') }}: <strong>' + data.input + '</strong></div>';
        },
        no_results: function() {
            return '<div class="no-results">{{ __('admin.contractors.form.area_no_results') }}</div>';
        }
    }
});

$('#edit-contractor-form').on('submit', function (e) {
    e.preventDefault();
    var btn = $('#submit-btn');
    ...
    axios.post('{{ route('admin.contractors.update', $contractor) }}', new FormData(this))
        .then(function (res) {
